...
class Cityscapes_active_AEL_loader(BaseDataset):
    """cityscapesLoader
    ...
    def __init__(
        ...
        self.acp = acp and self.split == "train"
        self.prob = prob
        self.acp = acp
    ...
    def __getitem__(self, index):
        ...
        if self.acp and random.random() > self.prob:
            paste_idx = random.randint(0, self.__len__() - 1)
            paste_img_path = self.files[paste_idx].rstrip()
            paste_img = Image.open(paste_img_path)

            paste_label_path = os.path.join(
                self.annotations_base,
                paste_img_path.split(os.sep)[-2],
                os.path.basename(paste_img_path)[:-15] + "gtFine_labelIds.png",
            )
            paste_label = Image.open(paste_label_path)

            paste_img = np.array(paste_img, dtype=np.uint8)
            paste_label = np.array(paste_label, dtype=np.uint8)
            paste_label = self.encode_segmap(np.array(paste_label, dtype=np.uint8))

            if self.augmentations is not None:
                img, lbl = self.augmentations(img, lbl)
                paste_img, paste_label = self.augmentations(paste_img, paste_label)

            if self.is_transform:
                img, lbl = self.transform(img, lbl)
                paste_img, paste_label = self.transform(paste_img, paste_label)

            return img, lbl, paste_img, paste_label, index
